...
last_updated: 2026-08-18 (rev18 — 木曜「脳・認知」のトピック多様性ルール追加。MRI×機械学習は最大3本)
---
...
19. **🔴 木曜（脳・認知）のトピック多様性 — MRI×機械学習への偏り禁止（rev18 / 2026-08-18 指示・最重要）**。

    **背景**：2026年6〜8月の木曜レポートが「脳MRI × 深層学習／機械学習」の予測モデル論文で占められ
    （brain age 予測、CNN/LSTM による診断、画像セグメンテーション等）、ユーザーから
    「MRIの機械学習についての論文が多すぎる。脳に関する知見について広く扱うように」と指摘された。
    木曜テーマは「脳・認知」であり、手法（MRI×ML）ではなく**脳と認知に関する知見全般**が対象である。

    **構成ルール（木曜の10本）**：
    - **MRI × 機械学習／深層学習の予測モデル論文は最大3本まで**（10本中4本以上は違反）
    - 残り7本以上は、以下のような**脳・認知の知見**から幅広く選ぶ：
      - 認知症・MCI の疫学（危険因子・予防・発症率の推移・社会的決定要因）
      - 介入研究（運動・認知トレーニング・栄養・睡眠・多因子介入の RCT）
      - 血液／CSF バイオマーカー（p-tau217、Aβ42/40、NfL、GFAP など）と臨床応用
      - 神経生理（EEG／MEG／TMS／fNIRS）・脳刺激・神経可塑性
      - 神経病理・剖検研究、動物・細胞レベルの機序（神経炎症、シナプス、グリア）
      - 認知の縦断コホート（認知予備能、教育歴、社会参加、難聴・視力）
      - 睡眠・概日リズムと脳、脳血管（WMH・微小出血）・血管性認知障害
      - 薬物治療（レカネマブ等の抗アミロイド薬、精神科薬）と臨床試験
      - 脳画像でも**機械学習を用いない**知見（構造・機能・代謝と行動/認知の関連）
    - **手法の新規性ではなく「脳・認知について何が分かったか」を優先**して選ぶ

    **他の曜日との住み分け**：MRI × 深層学習の手法論文は土曜（AI・データ科学）の担当領域。
    木曜で扱う場合は「その手法によって脳・認知の何が新しく分かったか」が主眼の論文に限る。

    **PD研究レポート枠にも同じ配慮**：PD枠は筋・脳・身体機能が主題だが、
    脳側の論文が画像 × ML に偏らないよう、機序・生理・介入・オミクスをバランスさせる。
